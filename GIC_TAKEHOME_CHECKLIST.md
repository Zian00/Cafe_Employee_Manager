...
- [ ] `GET /cafes?location=<location>`:
  - [ ] Return cafes sorted by `employees` desc.
  - [ ] Filter by location when valid location provided.
  - [ ] Return empty list if invalid location provided.
  - [ ] Return all cafes when no location.
  - [ ] Response fields: `name`, `description`, `employees`(int), `logo`(optional), `location`, `id`.
- [ ] `GET /employees?cafe=<cafe>`:
  - [ ] Return all employees sorted by `days_worked` desc.
  - [ ] If cafe provided, return only employees under that cafe.
  - [ ] Compute `days_worked` = current date - assignment start date.
  - [ ] Response fields: `id`, `name`, `email_address`, `phone_number`, `days_worked`(int), `cafe`(blank if unassigned).
- [ ] `POST /cafes`: create cafe.
- [ ] `PUT /cafes/{id}`: update existing cafe.
- [ ] `DELETE /cafes/{id}`: delete cafe and cascade delete employees under that cafe (as per your decision).
- [ ] `POST /employees`: create employee; cafe assignment is optional (employee can exist unassigned).
- [ ] `PUT /employees/{id}`: update employee and assignment.
- [ ] `DELETE /employees/{id}`: delete employee.
- [ ] Add API contract docs (OpenAPI/Swagger preferred).

## 5. Frontend Setup
- [ ] Initialize React app with Vite (preferred) or CRA.
- [ ] Add required libraries:
  - [ ] AgGrid for data tables.
  - [ ] Ant Design for UI framework (required by spec).
  - [ ] Tailwind CSS — use alongside Ant Design (disable Tailwind preflight in `tailwind.config.js` to prevent style conflicts with Ant Design).
  - [ ] Optional: TanStack Query for server state.
  - [ ] Optional: Day.js for date handling.
- [ ] Configure routing for:
  - [ ] `/cafes`
  - [ ] `/employees`
  - [ ] `/cafes/new`, `/cafes/:id/edit`
  - [ ] `/employees/new`, `/employees/:id/edit`
- [ ] Define reusable UI components (textbox, modal confirm, form actions).
- [ ] Apply custom styling beyond default framework theme.
